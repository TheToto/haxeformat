...
export class Unserializer {
    ...
    public static registerSerializableClass(clazz: new () => any) {
        let name = clazz.name
        if (name === undefined || name === null) throw new Error("Unable to get class name")
    ...
    public static registerSerializableEnum(enumz: typeof HaxeEnum) {
        let name = enumz.enum
        if (name === undefined || name === null) throw new Error("Unable to get enum name")
        if (name === "undefined" || name === "null" || name === "")
    ...
    protected unserializeEnum(
        edecl: typeof HaxeEnum | undefined | null,
        tag: string | number,
        ename: string
    ): HaxeEnum {
        this.pos++ /* skip ':' */
        ...
        if (!enumClass && !this.allowUnregistered) throw new Error("Unknown enum index/name : " + tag)
        let numArgs = this.readDigits()
        let args = Array(numArgs)
            .fill(0)
            .map((_) => this.unserialize())
        if (enumClass) {
    ...
    protected readFloat() {
        ...
        while (true) {
            ...
            if (this.isEof(c)) break
            // + - . , 0-9
            if ((c >= 43 && c < 58) || c === 101 /*"e"*/ || c === 69 /*"E"*/) this.pos++
            else break
        }
    ...
    public unserialize(): any {
        ...
        switch (get(this.pos++)) {
            ...
            case 114: // "r" class enum or structure reference
                let n = this.readDigits()
                if (n < 0 || n >= this.cache.length) throw new Error("Invalid reference")
                return this.cache[n]
            case 82: // "R" string reference
                let nn = this.readDigits()
                if (nn < 0 || nn >= this.scache.length) throw new Error("Invalid string reference")
            ...
            case 99: // "c" class instance
                ...
                if (!cli && !this.allowUnregistered) throw new Error("Class not found " + cname)
                let co: any = Object.create(cli?.prototype ?? Object.prototype) // creates an empty instance, no constructor is called
                if (!cli && this.addTypeHints) addTypeHint(co, cname)
                this.cache.push(co)
            ...
            case 119: // "w" enum instance by name
                ...
                if (!edecl1 && !this.allowUnregistered) throw new Error("Enum not found " + ename1)
            ...
            case 106: // "j" enum instance by index
                ...
                if (!edecl2 && !this.allowUnregistered) throw new Error("Enum not found " + ename2)
                this.pos++ /* skip ':' */
            ...
            case 108: // "l" haxe list to javascript array
                let l = new Array<any>()
                if (this.addTypeHints) addTypeHint(l, "List")
                this.cache.push(l)
                while (get(this.pos) !== 104 /* "h" */) l.push(this.unserialize())
                this.pos++
                return l
            case 98: // "b" string map
                let hsm: Record<string, any> = {}
                if (this.addTypeHints) addTypeHint(hsm, "StringMap")
                this.cache.push(hsm)
            ...
            case 113: // "q" haxe int map
                ...
                if (this.addTypeHints) addTypeHint(him, "IntMap")
                this.cache.push(him)
                let c = get(this.pos++)
                while (c === 58 /* ":" */) {
                ...
                if (c !== 104 /* "h" */) throw new Error("Invalid IntMap format")
                return him
            case 77: // "M" haxe object map
                let wm: any = { keys: [], values: [] }
                if (this.addTypeHints) addTypeHint(wm, "ObjectMap")
                this.cache.push(wm)
                while (get(this.pos) !== 104 /* "h" */) {
            ...
            case 115: // "s" Buffers
                ...
                let bytes = Buffer.from(
                    this.buf.substr(this.pos, bytesLen).replace(/%/g, "+").replace(/:/g, "/"),
                    "base64"
                )
                this.pos += bytesLen
            ...
            case 67: // "C" custom
                ...
                if (cl == null) throw new Error("Class not found " + name)
                ...
                if (get(this.pos++) !== 103 /*"g"*/) throw new Error("Invalid custom data")
                return cclo
